...
# TEMP: verify new key works
@router.post("/test-signing")
async def test_signing():
    ...
    emisor = get_billing_emisor()
    mh_creds = get_billing_mh_credentials()
    pem = mh_creds.get("private_key_pem", "")
    if not pem or len(pem) < 100:
        return {"error": "No valid PEM key", "pem_len": len(pem)}

    builder = DTEBuilder(emisor=emisor, ambiente="01")
    BILLING_ORG_ID = "35505aeb-7343-4d50-b098-f713239685c3"
    rec = {"tipo_documento": "36", "num_documento": "00000000000000", "nombre": "Test Firma",
           "direccion_departamento": "06", "direccion_municipio": "14",
           "direccion_complemento": "San Salvador", "telefono": "00000000", "correo": "test@test.com"}
    items = [{"precio_unitario": 1.0, "descripcion": "Test firma nueva llave", "codigo": "T001", "tipo_item": 1, "cantidad": 1}]

    try:
        dte, cg = builder.build(tipo_dte="01", numero_control="DTE-01-M001P001-000000000099999", receptor=rec, items=items)
        signed = SignEngine.sign_with_pem(pem, dte)
        # Auth
        nit = mh_creds["nit"]
        async with httpx.AsyncClient(timeout=30.0, verify=True) as client:
            auth_resp = await client.post("https://api.dtes.mh.gob.sv/seguridad/auth", data={"user": nit, "pwd": mh_creds["password"]})
        body = auth_resp.json().get("body", auth_resp.json())
        ...
        result = await ts.transmit(token_info=token_info, signed_dte=signed, tipo_dte="01", codigo_generacion=cg)
        return {"success": True, "estado": result.status, "sello": result.sello_recepcion}
    except Exception as e:
        obs = getattr(e, "observaciones", [])
        return {"success": False, "error": str(e), "observaciones": obs}
